...
fsdir_t * submission_package_byfile(const submission_package_t * subp, const char * filename, const bool pfs) {
    for (size_t i = 0; i < list_size(subp->nca_list); i++) {
        const content_archive_t * nca = list_get(subp->nca_list, i);
        for (size_t fs_index = 0; content_archive_get_fs(nca, fs_index, pfs); fs_index++) {
            fsdir_t * fs_file = content_archive_get_fs(nca, fs_index, pfs);

            vector_t *files = fs_list_all_files(fs_file);
            bool exist = false;
